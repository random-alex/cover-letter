...
def get_cover_letter_openai(cv: str, job_description: str, is_pdf_cv: bool, is_url_jd: bool, gpt_api_key: str = None):
    if not gpt_api_key:
    ...
    embedding = OpenAIEmbeddings(openai_api_key=gpt_api_key)

    cv_info = get_structured_cv_info(cv=cv, is_pdf_cv=is_pdf_cv, model=model, embedding=embedding)
    job_description = get_structured_jd_info(job_description=job_description, is_url_jd=is_url_jd, model=model,
                                             embedding=embedding)
...
if __name__ == '__main__':
    ...
    cover_letter = get_cover_letter_openai(cv=args.cv, job_description=args.job_description, is_pdf_cv=args.is_pdf_cv,
                                           is_url_jd=args.is_url_jd)
